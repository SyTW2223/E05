...
let Serie = require('../src/models/serie.model');
...
describe('API SERIE succes', () => {
  ...
  let serie2 = {
    ...
    it('Should successfully get a serie2', async () => {
      await supertest(app).get(`/serie/${serie.title}`).send(2).expect(200);
    });
...
describe('API serie errors', () => {
  ...
  it('Should error at insert a new serie', async () => {
    await supertest(app).post('/series').send(serieTestError).expect(500);
  });
